...
def uNet(input, time, lat, lon, height, kernel = [5, 3, 3], nodes = [72, 144, 288, 576]):#uNet(input, time, lat, lon, height, kernel = [5, 3, 3], nodes = [72, 144, 288, 576]):
    '''
...
def get_model(PS=32, loss = grad_loss, optimizer = 'adam', nodes = [72, 144, 288, 576], residual = False):#PS=32
    '''
    ...
    TS = 3 #added
    # Input layers
    main_input  = Input(shape = (TS, PS, PS, 1))
    time        = Input(shape = (TS, int(PS/8), int(PS/8), 1))
    lat         = Input(shape = (TS, PS, PS, 1))
    lon         = Input(shape = (TS, PS, PS, 1))
    height      = Input(shape = (TS, PS, PS, 1))

    # Load U-Net
    unet        = uNet(main_input, time, lat, lon, height, nodes = nodes)#uNet(main_input, time, lat, lon, height, nodes = nodes)
    ...
    #[main_input, time, lat, lon, height],
